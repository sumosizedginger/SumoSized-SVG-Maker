  ...
  renderedSvg = $derived.by(() => {
    ...
    try {
      ...
      for (const layer of this.layers) {
        ...
        const sx = tr.scaleX !== undefined ? tr.scaleX : (tr as any).scale || 1;
        const sy = tr.scaleY !== undefined ? tr.scaleY : (tr as any).scale || 1;
        ...
        this.layers = data.l.map((l: any) => ({
          ...
          transforms: l.t
            ? {
                x: l.t.x || 0,
                y: l.t.y || 0,
                scaleX: l.t.scaleX !== undefined ? l.t.scaleX : l.t.scale || 1,
                scaleY: l.t.scaleY !== undefined ? l.t.scaleY : l.t.scale || 1,
    ...
    for (let i = 0; i < count; i++) {
      const variantLayers = this.layers.map((layer) => {
        ...
        for (const paramDef of gen.params) {
          if (paramDef.type === "number" || paramDef.type === "integer") {
            ...
            let val =
              (variantParams[paramDef.name] ?? paramDef.default) + jitter;
      ...
      try {
        ...
        for (const layer of variantLayers) {
          ...
          const sx =
            tr.scaleX !== undefined ? tr.scaleX : (tr as any).scale || 1;
          const sy =
            tr.scaleY !== undefined ? tr.scaleY : (tr as any).scale || 1;
          const transformAttr = `transform="translate(${tr.x}, ${tr.y}) scale(${sx}, ${sy}) rotate(${tr.rotation}, 50, 50)"`;
