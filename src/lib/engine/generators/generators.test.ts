		...
		describe(`Generator [${gen.id}]`, () => {
				...
				gen.params.forEach(p => {
					if (p.type === 'select' && p.options) {
						p.options.forEach(opt => {
							const params = { ...gen.defaultParams, [p.name]: opt.value };
							const svg = gen.render(params, 12345);
							expect(typeof svg).toBe("string");
							expect(checkA11y(`<svg viewBox="0 0 100 100"><title>MOCK</title><desc>MOCK</desc>${svg}</svg>`).valid).toBe(true);
						});
					} else if (p.type === 'boolean') {
						const paramsTrue = { ...gen.defaultParams, [p.name]: true };
						const svgTrue = gen.render(paramsTrue, 12345);
						expect(checkA11y(`<svg viewBox="0 0 100 100"><title>MOCK</title><desc>MOCK</desc>${svgTrue}</svg>`).valid).toBe(true);

						const paramsFalse = { ...gen.defaultParams, [p.name]: false };
						const svgFalse = gen.render(paramsFalse, 12345);
						expect(checkA11y(`<svg viewBox="0 0 100 100"><title>MOCK</title><desc>MOCK</desc>${svgFalse}</svg>`).valid).toBe(true);
						...
						if (p.min !== undefined && p.max !== undefined) tests.push((p.max + p.min) / 2);
						if (tests.length === 0) tests.push(0, 10, 100);

						tests.forEach(testVal => {
							const paramsNum = { ...gen.defaultParams, [p.name]: testVal };
							const svgNum = gen.render(paramsNum, 12345);
							expect(checkA11y(`<svg viewBox="0 0 100 100"><title>MOCK</title><desc>MOCK</desc>${svgNum}</svg>`).valid).toBe(true);
						});
					}
				});
			});
			...
			it("fuzzes schema parameters safely to execute internal logic branches", () => {
				...
				for (let i = 0; i < 5; i++) {
					...
					gen.params.forEach(p => {
						if (p.type === 'boolean') fuzzed[p.name] = Math.random() > 0.5;
						else if (p.type === 'select' && p.options) fuzzed[p.name] = p.options[Math.floor(Math.random() * p.options.length)].value;
						else if (p.type === 'number') fuzzed[p.name] = p.min !== undefined ? p.max !== undefined ? (p.max + p.min) / 2 : p.min * 2 : 10;
